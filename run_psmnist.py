...
from src.PSMNIST.trainer import *
from src.PSMNIST.snn import *
...
if __name__ == "__main__":
    ...
    for run_seed in seed_list:
        ...
        wandb.init(
                project= 'PSMINST',
                config=cfg,
                name=f"perf_PSMNIST/{model_name}/{config.dataset}_{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            )
        ...
        perm_path = os.path.join(config.results_dir, "perm.pt")
        ...
        for epoch in range(config.epochs):
            ...
            train_acc, train_loss = train(train_loader, model, optimizer, epoch, device, config, perm)
            val_acc, val_loss = test(valid_loader, model, epoch, device, config, perm)
            ...
            val_res.to_csv(os.path.join(config.results_dir, 'train_res.csv'), index=True)
        ...
        final_test_acc, final_test_loss = test(
            test_loader, model, epoch, device, config, perm
        )
