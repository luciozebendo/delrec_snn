...
class SNN(torch.nn.Module):
    def __init__(self, config):
        ...
        for idx, layer_dim in enumerate(config.hidden_layers):
            ...
            layers.append(nn.Dropout(config.feedforward_dropout_rate))
    ...
    def forward(self, x): 
        assert x.dim() == 3 # (T, B, N)
        x = self.layers(x)
        x = self.layers[-1].v_seq  # return last layer's voltage sequence # (T, B, N)
        return x
...
class SNN_recurrent_delays(SNN):
    ...
    def clamp_delays(self):
        for m in self.layers:
            if isinstance(m, SNNTorchAxonalRecDel) or isinstance(m, ConvSNNTorchAxonalRecDel) or isinstance(m, axonal_recdel):
    ...
    def log_params(self):
        ...
        logs = {}
        for idx, layer in enumerate(self.layers):
            if isinstance(layer, SNNTorchAxonalRecDel) or isinstance(layer, ConvSNNTorchAxonalRecDel) or isinstance(layer, axonal_recdel):
