...
if __name__ == "__main__":
    ...
    for current_seed in seed:
        ...
        seed_everything(seed=config.seed, is_cuda=True)
         ...
         #COMMENTED OUT: GPU not used for now
        ...
        count_parameters(model) # print the number of trainable parameters.
        ...
        wandb.init(
                project= 'SSC', 
                config=cfg,
                name=f"{model_name}/{config.dataset}_{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            )

        # record the initial state of learnable delays to track its evolution over epochs
        ...
        # configure results directory and save config parameters
        ...
        for epoch in range(config.epochs):
            ...
            train_loader.reset() # reset data loaders for the new epoch
            valid_loader.reset()
            
            # update the dynamic parameters, like sigma
            for m in model.layers:
                if isinstance(m, axonal_recdel):
            ...
            # write training and validation metrics
            ...
            # compute and log the change in learnable delays from the previous epoch
            ...
            # make a dictionary with the current training state
            state = {
                'epoch': epoch,
                'acc': float(val_acc),
                'model': getattr(model, 'module', model).state_dict(),
                'optim': [opt.state_dict() for opt in optimizer],
                'sched': [sch.state_dict() for sch in scheduler],
                'rng': {
                    'torch': torch.get_rng_state(),
                    'cuda': torch.cuda.get_rng_state_all() if torch.cuda.is_available() else None,
                },
            }
            # save the last model state and update the best model if the validation accuracy improves
        ...
        # after training, load the best performing model for final testing on unseen data
        best_ckpt = torch.load(os.path.join(config.results_dir, 'best.pth'))
        model.load_state_dict(best_ckpt['model'])
        best_epoch = best_ckpt['epoch']
        best_val   = best_ckpt['acc']
        
        # use the best model on the unseen test dataset
        test_loader.reset()
        final_test_acc, final_test_loss = test(
            test_loader, model, epoch, device, config, 
        )

        # final test results for this seed
        print(f"\nFinal Test (seed {current_seed}) ► Acc: {final_test_acc:.4f}, "
